...
for g in range(len(length_Lh_min)-1):
	...
	for j in range(0,trial_no,1):
		spikes = np.load(currentPath+'/spikes_'+str(first_spl)+'i'+str(second_spl)+'f_dB_'+str(freq)+'Hz_Lumax'+str(length_Lu_max[g])+'_Lumin'+str(length_Lu_min[g])+'_Lhmax'+str(length_Lh_max[g])+'_Lhmin'+str(length_Lh_min[g])+'.npy',allow_pickle=True)[j]
...
#		import pdb; pdb.set_trace()
	...
	if path.exists("plot.pickle"):
		...
		xmax=4.75
		plt.xlim(xmin, xmax)
		pf=Rectangle((1,-410),2.5,25,facecolor='gray',edgecolor='None')
		ax.add_patch(pf)
		plt.text(2,-408,'Noise only',fontsize=9,color='w')
		pe=Rectangle((3.5,-410),2.5,25,facecolor='green',edgecolor='None')
		ax.add_patch(pe)
		plt.text(4,-408,'Sound with Noise',fontsize=9,color='w')
		plt.ylim(-410,65)
	...
	plt.savefig(currentPath+'/thenew_convolution_'+str(first_spl)+'i'+str(second_spl)+'f_dB_'+str(freq)+'_Hz.png',dpi=300)
